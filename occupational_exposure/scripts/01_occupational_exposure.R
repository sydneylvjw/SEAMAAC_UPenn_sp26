...
oes_in4_zip <- cache_download(config$oes_in4_url, basename(config$oes_in4_url))
oes_in4_dir <- file.path(config$cache_dir, tools::file_path_sans_ext(basename(config$oes_in4_url)))
if (!dir.exists(oes_in4_dir)) utils::unzip(oes_in4_zip, exdir = config$cache_dir)
