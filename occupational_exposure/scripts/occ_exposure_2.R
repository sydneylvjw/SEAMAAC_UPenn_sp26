...
#     `02_occupational_exposure_pums.R`.
#
...
# ---------------------------------------------------------------------------
.workspace <- file.path(dirname(dirname(getwd())), "SEAMAAC HVI")
...
config <- list(
  ...
  cache_dir  = file.path(.workspace, "occupational_exposure", "data_cache"),
  data_in    = file.path(.workspace, "occupational_exposure", "data_in"),
  output_dir = file.path(.workspace, "occupational_exposure", "output")
)
...
oes_national_path <- file.path(config$data_in, "OES_Report.xlsx")
if (!file.exists(oes_national_path)) {
  stop(
    "Missing OES_Report.xlsx. Place the BLS OES national report at: ",
    oes_national_path
  )
}

oes_national <- readxl::read_xlsx("/Users/sydneyjones/Documents/Claude/Projects/SEAMAAC HVI/occupational_exposure/data_in/OES_Report.xlsx", sheet = "OES Sheet") |>
...
oes_in4_file <- list.files(
  "SEAMAAC_practicum/occupational_exposure/data/raw/oesm23in4/",
  pattern = "nat4d.*\\.xlsx$", full.names = TRUE, recursive = TRUE
)[1]
stopifnot(!is.na(oes_in4_file))

oes_industry <- readxl::read_xlsx("/Users/sydneyjones/Documents/MCPMUSA/R_projects/SEAMAAC_UPenn_sp26/SEAMAAC_practicum/occupational_exposure/data/raw/oesm23in4/nat4d_owner_M2023_dl.xlsx") |>
  janitor::clean_names() |>
...
tract_geom <- dp03_parsed |>
  dplyr::filter(variable == "DP03_0004E") |>
